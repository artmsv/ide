    ...
    fn add_node_and_edit(&self) -> NodeId {
            ...
            self.graph_editor.model.nodes.selected.first_cloned()
        {
            let selected_pos = self
                .graph_editor
                .model
                .get_node_position(selected)
                .unwrap_or_default();
            let styles = StyleWatch::new(&self.app.display.scene().style_sheet);
            let offset_y = styles
                .get_number(ensogl_theme::project::default_gap_between_nodes);
...
impl View {
    ...
    pub fn new(app: &Application) -> Self {
        ...
        let default_gap_between_nodes_path =
            ensogl_theme::project::default_gap_between_nodes;

        let default_gap_between_nodes =
            styles.get_number_or(default_gap_between_nodes_path, 0.0);
        frp::extend! { network
            frp.source.default_gap_between_nodes <+ default_gap_between_nodes;
        }
        frp.source
            .default_gap_between_nodes
            .emit(default_gap_between_nodes.value());
