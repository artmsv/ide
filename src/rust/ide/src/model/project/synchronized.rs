...
impl ExecutionContextsRegistry {
    ...
    pub fn handle_expression_updates(
        &self,
        update: language_server::ExpressionUpdates,
    ) -> FallibleResult {
...
impl UnsupportedEngineVersion {
    fn error_wrapper(
        ...
        move |root_cause| {
            let requirements = semver::VersionReq::parse(
                controller::project::ENGINE_VERSION_SUPPORTED,
            );
            match requirements {
...
impl Display for UnsupportedEngineVersion {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        let version_supported =
            controller::project::ENGINE_VERSION_FOR_NEW_PROJECTS;
        ...
        move |event| {
            ...
            match event {
                Event::Notification(Notification::ExpressionUpdates(
                    ...
                    {
                        let result = execution_contexts
                            .handle_expression_updates(updates);
                        if let Err(error) = result {
